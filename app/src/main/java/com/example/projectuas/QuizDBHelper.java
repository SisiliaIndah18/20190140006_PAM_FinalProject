...
public class QuizDBHelper extends SQLiteOpenHelper {
    ...
    private static final String DB_NAME = "quizzes.db"; //nama db
    private static final int DB_VERSION = 8; //ver db

    public static final String CATEGORY_MTK = "matematika"; //deklarasi variabel dengan jenis data string
    public static final String CATEGORY_IPA = "ipa"; //deklarasi variabel dengan jenis data string
    public static final String CATEGORY_IPS = "ips"; //deklarasi variabel dengan jenis data string
    public static final String CATEGORY_AGAMA = "agama"; //deklarasi variabel dengan jenis data string
    public static final String CATEGORY_OLAHRAGA = "olahraga"; //deklarasi variabel dengan jenis data string
    public static final String CATEGORY_PROGRAM = "pemrograman"; //deklarasi variabel dengan jenis data string

    //method membuat tabel query untuk pertanyaan dan jawaban
    ...
    private final String DROP_TABLE_QUERY = "DROP TABLE IF EXISTS " + QuestionsTable.TABLE_NAME; //methode drop tabel query

    private SQLiteDatabase db; //deklarasi variabel untuk SQLiteDatabase
    private List<Question> mQuestionList; //deklarasi variabel dengan jenis data list

    private Bundle categoryIntentBundle; //membuat method bundle

    //membuat kostruktor QuizDBHelper
    public QuizDBHelper(Context context, Bundle bundle) {
        super(context, DB_NAME, null, DB_VERSION);
        this.categoryIntentBundle = bundle; //memberikan nilai categoryIntentBundle dengan bundle
    }

    //membuat kosntruktor onCreate
    @Override
    public void onCreate(SQLiteDatabase db) {
        this.db = db; //memberikan nilai db dengan db
        db.execSQL(CREATE_TABLE_QUERY); //untuk execute create tabel query

        setUpQuestions();
        insertQuestions();
    }

    //membuat kostruktor onUpgrade
    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        db.execSQL(DROP_TABLE_QUERY); //untuk execute drop tabel query
        onCreate(db);
    }

    //methode set pertanyaan
    private void setUpQuestions() {
        mQuestionList = new ArrayList<>(); //membuat objek dari mQuestionList menjadi ArrayList

        //pertanyaan untuk agama
    ...
    //method untuk memasukkan pertanyaan
    ...
    //membuat kosntruktor ArrayList, fungsi ini untuk mengambil pertanyaan dan pilihan jawaban
    public ArrayList<Question> getAllQuestions(String categoryID) {
        Log.d("TAG", "Getting all questions for : " + categoryID);
        ArrayList<Question> questionList = new ArrayList<>(); //membuat objek baru
        db = getReadableDatabase(); //deklarasi db
        String SELECT_TABLE_QUERY = "SELECT * FROM " + QuestionsTable.TABLE_NAME + " WHERE " + QuestionsTable.COLUMN_CATEGORY + " = \"" + categoryID + "\""; //query db
        ...
        cursor.close();
        return questionList; //kembali ke questionList
    }
}